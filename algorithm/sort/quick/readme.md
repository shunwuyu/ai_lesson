...
数组：[2a, 2b, 1]，两个 2 区分下标 a、b。
以 1 为基准快排一趟结果：[1, 2b, 2a]
原本2a在2b前面，排序后颠倒，相等元素位置改变，证明快排不稳定。

那些数组排序算法
- 冒泡排序：相邻两两交换，慢，适合少量数据
- 插入排序：逐个插入有序区，近乎有序数组效率极高 插牌
- 选择排序：每次选择剩余元素中最小的，放到有序区，慢，适合少量数据