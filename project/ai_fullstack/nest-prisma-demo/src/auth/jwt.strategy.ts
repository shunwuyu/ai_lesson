// jwt.strategy.ts
// jwt.strategy.ts 用于在 NestJS 中实现基于 JWT 的身份认证策略
// 自动解析请求中的 Token 并验证用户身份，从而保护受鉴权路由。
// Strategy 是用于定义 JWT 鉴权逻辑的策略类；
// ExtractJwt 提供方法（如从 Bearer Token 中）提取 JWT 令牌。
// passport-jwt 是一个 Passport 策略插件，用于在 Node.js 应用中解析并
// 验证 HTTP 请求中的 JWT（JSON Web Token），实现基于令牌的身份认证。
