...
所以在编写 Web 应用时，你还需要衡量这些事件所占用的时长（react render），并想办法解决单个任务占用主线程过久的问题。
...
- 第一个问题是如何处理高优先级的任务。
    用户点击、滚动、动画
    在添加到消息队列的过程中，可能前面就有很多任务在排队了，

    针对这种情况，微任务就应用而生了。

    通常我们把消息队列中的任务称为宏任务，每个宏任务中都包含了一个微任务队列（允许插队）
...
- 第二个是如何解决单个任务执行时长过久的问题。
    ...
    ![](https://tukuimg.bdstatic.com/processed/bd2135c7e1ff4018c8891944bf996827.jpg)

    ![](https://p3-flow-imagex-sign.byteimg.com/labis/c0bc8ba5006943404063bed80cabf350~tplv-a9rns2rl98-pc_smart_face_crop-v1:512:384.image?lk3s=8e244e95&rcl=202603121030475D98CDAAE821F833EC34&rrcfp=cee388b0&x-expires=2088642662&x-signature=1TDIJjzjU9f%2Fpz97FNrEtJ7RhAw%3D)

    执行一个宏任务
    清空微任务队列
    执行页面渲染（Style → Layout → Paint → Composite）
    进入下一轮循环
