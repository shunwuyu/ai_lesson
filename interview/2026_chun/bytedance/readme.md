...
## 说说 ReAct 框架，Thought-Action-Observation 各是什么？
...
假设任务是"帮我查明天北京到上海的航班，选最便宜的经济舱"。

如果让 LLM 直接输出 Action，它可能会这样做：第一次调用：输出 {"tool": "search_flight", "from": "北京", "to": "上海", "date": "明天"}。看起来没问题，

但"明天"不是合法的日期格式，API 调用失败。

第二次调用：修正为 {"tool": "search_flight", "from": "北京", "to": "上海", "date": "2026-04-24"}。

这次成功了，返回 10 个航班。

第三次调用：LLM 看到 10 个航班，

直接输出 {"tool": "book_flight", "flight_id": "CA1234"}。

但它没有比价，选的不是最便宜的，任务失败。

问题出在哪里？LLM 没有机会"停下来想一想"。

它看到任务就输出工具调用，看到结果就输出下一个工具调用，中间没有推理过程，所以会出现：
...
这就是为什么 ReAct 要加 Thought 步骤。Thought 不是给人看的日志，而是给 LLM 自己看的推理过程。LLM 在 Thought 里写"当前状态是什么、下一步应该做什么、为什么这样做"，这个过程强迫它把任务拆解清楚，然后再输出 Action。类比一下，直接输出 Action 就像你让一个人"去超市买东西"，他冲进去随便拿了几样就结账，回来发现买错了。ReAct 是让他先列购物清单（Thought），再去拿东西（Action），拿完检查一遍（Observation），确认没问题再结账。

![](b.png)

### ReAct 的三个步骤到底是什么？

ReAct 的核心是一个三步循环：Thought、Action、Observation。

每一步都有明确的作用，缺一不可。

Thought（推理）： Agent 分析当前状态，决定下一步做什么。

这一步 LLM 输出的是自然语言，不是代码或 JSON。

典型的 Thought 长这样："当前状态：用户要查明天北京到上海的航班。我需要先确定明天的日期是 2026-04-24，然后调用 search_flight 工具，参数是 from=北京, to=上海, date=2026-04-24。"Thought 的作用是让 LLM 把推理过程显式写出来。写的过程就是思考的过程，LLM 会检查"我现在知道什么、还缺什么、下一步该做什么"。

这个机制来自 Chain-of-Thought，研究表明让 LLM 写出推理步骤可以显著提升复杂任务的准确率。Action（执行）： Agent 调用工具，执行具体操作。这一步 LLM 输出的是结构化指令，通常是 JSON 格式的工具调用。典型的 Action 长这样：
...
http://localhost:3000/agent/flight?q=帮我查明天北京到上海的航班，选最便宜的经济舱

## vite的打包原理

### 介绍

Vite 是尤雨溪团队（vue作者）开发的现代前端构建工具，依托原生 ESM 与 esbuild。工程化上实现秒级冷启动、毫秒级 HMR 热更新，配置简洁、开箱即用，生产打包高效轻量化，大幅提升开发与构建效率。

<script type="module" src="/src/main.js"></script>

现代主流浏览器已原生支持 ES Module (ESM), 无需打包即可浏览器直接运行文件，按需加载模块，省略编译打包，实现极速启动与即时热更新。

Webpack 会把零散的 js、组件、依赖全部整合压缩，打包成少数几个浏览器能识别的文件。传统方案必须先整体打包才能运行，启动慢、更新也要重新编译。

Webpack 会分析模块依赖关系，梳理执行顺序，递归整合所有 JS 资源，统一封装打包为浏览器可直接运行的代码

esbuild 是用 Go 语言写的极速 JS/CSS 打包压缩工具，比 Webpack 快 10-100 倍。Vite 用它做依赖预构建、TS/JSX 转译、代码压缩。

Webpack 需全量打包再启动；Vite 借助原生 ESM，无需预打包，直接按需加载实现秒级启动。

Vite 借助原生 ESM，仅更新修改模块，无需全量重编译，实现极速热更新。

Webpack 需复杂配置，Vite 开箱即用，零配置就能跑，按需少量修改即可满足开发需求。

### Vue3+TS+Stylus + 路由懒加载 说下vite 过程

一、开发环境启动阶段

1. 执行vite dev，Vite 先用esbuild预构建第三方依赖（Vue、Vue-Router 等 ），解决浏览器 ESM 裸导入问题，缓存依赖提升二次启动速度。
2. 本地启动静态服务，不做全局打包，直接基于浏览器原生 ESM 规范运行项目。

二、项目源码解析与转译
1. 浏览器请求入口main.ts，通过type="module"识别 ESM 模块。
2. 遇到 TS、Vue 单文件、Stylus 文件：交由 esbuild 实时转译，TS 编译为 JS、Stylus 编译为普通 CSS，按需单独编译、即时返回。
3. 全程无全量合并编译，做到秒级冷启动。

三、路由懒加载执行
路由中() => import('@/views/xxx.vue')，天然利用 ESM 动态import()能力。访问对应路由时，浏览器才异步请求该页面模块，实现按需加载、分割代码。

四、HMR 热更新流程

修改 Vue 组件、样式、TS 代码后，Vite 精准监听文件变化。只重新编译当前修改的单个模块，通过 HMR 通信局部替换更新，不刷新页面、保留页面状态，毫秒级生效。

五、生产打包（vite build）

切换生产环境，舍弃 esbuild 全量编译，交由 Rollup 统一打包。完成代码合并、Tree-Shaking、压缩、静态资源优化、路由懒加载代码分割，输出高性能线上产物。

Rollup 是专注库 / 项目的模块化打包工具，擅长 Tree-Shaking、代码精简。Vite 生产环境用它，打包输出高性能、体积更小的线上代码。
...
Tree-Shaking 会自动剔除从未引入、从未执行的 funB，打包时只保留有效代码，减小包体积。

压缩即移除空格、注释、缩短变量名，混淆精简代码，减小文件体积，加快线上加载速度。


压缩图片、字体等资源，自动添加文件哈希指纹（版本号）、按需分包缓存，减少请求体积、提升加载速度。

## TCP和UDP的区别，TCP协议之上的一些额外的协议有哪一些，UDP比TCP快在哪? 为什么http3.0 要改用udp ?

一、TCP 与 UDP 核心区别

连接性：TCP 面向连接，三次握手建立连接；
UDP 无连接，直接发包无需建立链路。

可靠性：TCP 可靠传输，丢包重传、有序、校验纠错；
UDP 不可靠，不保证送达、顺序。

控制机制：TCP 拥塞控制、流量控制；
5.1 来了， 人墙？ 拥塞控制是看外面的路堵不堵 
流量控制是看对方的嘴忙不忙
UDP 无额外控制，只管发送。

开销：TCP 头部大、开销高；
TCP 头部确实比 UDP（仅 8 字节）大得多，其固定头部就有 20 字节，加上可选字段后最大可达 60 字节
源端口与目的端口（各 16 位）：相当于快递的“寄件人”和“收件人”门牌号，用于精准定位主机上的具体应用程序。
序列号（32 位）：给每个发送的数据字节打上编号，确保数据能按顺序重组，防止乱序。
确认号（32 位）：告诉发送方“我已经收到了哪些数据，下一个该发什么”，是可靠传输的核心。
校验和（16 位）：用于检测数据在传输过程中是否损坏，保障数据的完整性。
窗口大小（16 位）：告诉对方“我还能接收多少数据”，是实现流量控制、防止接收方被撑爆的关键。

UDP 头部极简，开销极低。

二、基于 TCP 的上层协议
HTTP/1.1、HTTP/2、HTTPS、FTP、SSH、SMTP、DNS (部分)、MySQL 数据库协议等。

三、UDP 比 TCP 快在哪
无需握手建连、挥手断连，省去连接开销；
无丢包重传、排序、拥塞控制等复杂校验逻辑；
协议头部更小，传输冗余少，延迟更低、吞吐更高。

四、HTTP3 改用 UDP 的原因
TCP 存在队头阻塞，单个丢包会阻塞后续所有数据，网络弱网体验差；
基于 UDP 自研 QUIC 协议，既能自定义可靠传输，又规避 TCP 阻塞；
QUIC 内置加密、0-RTT 快速建连，握手更快，移动端、弱网环境延迟大幅降低。
