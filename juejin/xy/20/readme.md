...
- 如何对数组排序？
  JS 天生地提供了对排序能力的
  ```
  Array.prototype.sort() 是 JavaScript 数组的排序方法
  原地排序：直接修改原数组（不会创建新数组），返回值是排序后的原数组（引用相同）；
  默认排序规则（无参数）
  默认将数组元素 转换为字符串，然后按「UTF-16 字符编码值」从小到大排序。
  const arr = [10, 2, 30, 5];
  arr.sort(); // 结果：[10, 2, 30, 5]// 按编码
  二、比较函数 「比较函数」compareFunction(a, b)，它的作用是定义排序规则。
  a 和 b 是数组中正在被两两对比的两个元素（排序算法会遍历数组，不断取两个元素对比）
  返回值 < 0	a 排在 b 前面（升序）
  返回值 = 0	a 和 b 相对位置不变（ES2019 后稳定）
  返回值 > 0	b 排在 a 前面（降序）
  a - b 的本质是通过返回值的正负，强制数字按「数值大小」升序排列
  arr.sort((a,b) => {
    return a - b
  })
  反向：(a,b) => b - a 实现数字降序
  ...
  const arr = ["10", "2", "30", "5"];
arr.sort((a, b) => a - b); // [2, 5, 10, 30]（自动转数字）

  const arr = [{name: 'A', age: 20}, {name: 'B', age: 18}, {name: 'C', age: 20}];
  arr.sort((a, b) => a.age - b.age); 
  // 结果：B(18) → A(20) → C(20)（A和C的相对位置不变）
  ```
...
在冒泡排序的过程中，有一些”动作“是不太必要的。
- 比如数组在已经有序的情况下，为什么还要强行再从头到尾再对数组做一次遍历？
- 随着外层循环的进行，数组尾部的元素会渐渐变得有序， 没有必要比较
...
排序算法	优点	缺点
冒泡排序	实现简单，稳定，可检测有序提前结束	时间复杂度高（O(n²)），效率低
选择排序	交换次数少（最多 n−1 次），实现简单	不稳定，时间复杂度始终 O(n²)，无法提前优化
插入排序	对小规模或近似有序数据效率高，稳定，原地排序	数据逆序时性能差，最坏 O(n²)

### 稳定性

[5₁, 2, 5₂, 1]

此时两个 5 的相对顺序从 5₁ 在前、5₂ 在后 变成了 5₂ 在前、5₁ 在后！
虽然值相等，但它们的原始先后顺序被颠倒了，这就违反了“稳定性”——相等元素排序后相对位置不变。