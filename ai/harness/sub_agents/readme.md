...
Harness 里的 SubAgent 就是子代理，你可以理解为主 Agent 手下的小帮手。主 Agent 接到复杂任务后，拆出细分工作交给 SubAgent 单独去跑。每个 SubAgent 有独立对话空间，干活的中间过程不会污染主 Agent 上下文，干完只返回最终结果。还能多个并行执行，任务完成就销毁，这样主 Agent 不用被一堆细节拖累，专注统筹和汇总。
