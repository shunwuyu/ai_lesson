...
import requests
import torch
# __name__ 当前模块的名称 内置变量
app = Flask(__name__)
cache_dir = './llama_cache'
model_path = cache_dir + '/LLM-Research/Qwen2-0___5B-Instruct'
...
def call_expert(url, task_requirement):
  response = requests.post(url, json={"intent":task_requirement}, timeout=500)
  response.raise_for_status
  return response.json().get("response", "Error: No response from expret")
# 匿名函数
...
law = Tool.from_function(func=ai_expert, name="law_expert", description="当你需要法律合规专家知识时使用这个工具，输入为具体问题，返回为问题答案")
