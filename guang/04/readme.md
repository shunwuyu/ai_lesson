...
这些 tool 怎么调用、参数是什么都是大模型**自己决定**的。

tool 给大模型扩展了做事情的能力，本来它只能思考，不能做事情，但是现在可以自己调用 tool 来帮你做事情了。但你有没有发现 tool 有个**问题**：
...
现在是解决了跨语言调用工具的问题。 
那如果每个人都这么搞， 他们提供的服务都不一样， 想接入别的tool, 是不是要了解每个服务都是怎么定义的呢？

能不能定义一个统一的通信协议， 我们都按这个格式沟通， 这样所有的跨进程调用就可以接入。 

## MCP 

想跨进程调用某个工具，通过这个MCP协议通信就行。

不管是本地工具， 直接跑哪个进程， 然后stdio 通信。

远程工具， 通过http 链接远程服务进程。
...
```
{
  "my-mcp-server": {
    "command": "node",
    "args": [
      "/Users/shunwuyu/workspace/lesson/ai_lesson/guang/04/tool-test/my-mcp-server.js"
    ]
  }
}
```

请使用my-mcp-server 001 用户信息查询

查看一下MCP Server 的使用指南， 总结一下它的核心功能。