...
左边是 MySQL 数据库里的users表和posts表，右边是代码里对应的User类和Post类。ORM 会自动把类和表映射起来，我们不用写原生 SQL，直接调用userRepository、postRepository里的find、save、delete这些方法，就能操作数据库了，省心又安全。

typeorm

TypeORM 是一款支持 TypeScript 和 JavaScript 的 Node.js ORM 框架，能让你用面向对象的方式操作 MySQL、PostgreSQL 等多种数据库，无需编写原生 SQL。

pnpm install --save @nestjs/typeorm typeorm mysql2

先加一个用户，名字是乐乐，邮箱是11231221@qq.com
添加李四、王五，邮箱分别是lisi@xx.com、wangwu@xx.com
...
cron：提供 Cron 表达式解析能力
@nestjs/schedule：NestJS 官方定时任务模块，封装 cron，适配 Nest 生态实现定时任务。
pnpm install --dev @types/cron

除了 cron 类型的定时任务外，还有 timeout、interval 类型

首先我们要创建一个定时任务表来保存所有定时任务。比如豆包的定时任务有列表，删除功能：
