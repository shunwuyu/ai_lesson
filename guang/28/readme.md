...
我们会把原始数据存Mysql， 把需要检索的部分同步到Es 里。
...
Dockerfile 是文本脚本，写构建指令，用来自动打包生成 Docker 镜像。

为什么可以拉取镜像还要构建镜像？
直接`docker pull`拿到的**原版 ES 镜像**：只有 ES 本身，**不带 IK 中文分词器**。
通过 Dockerfile**构建新镜像**：在原版基础上提前装好插件


**analyzer：写入文档建索引时用的分词器；search_analyzer：查询搜索时用的分词器。**

- `analyzer:ik_max_word`：存数据，拆出尽可能多的词，扩大召回。
- `search_analyzer:ik_smart`：用户搜索词只做粗粒度最优分词，减少无关匹配。

### 通俗例子

文档：`人工智能实战`

1. **写入（ik_max_word）** 拆出：人工智能、人工、智能、实战。索引里全部保存，不漏词。
2. **用户搜 “人工智能”（ik_smart）** 只拆成【人工智能】一个词，不会拆碎，匹配更精准。